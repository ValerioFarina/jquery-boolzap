...
$(document).ready(function () {
  ...
  $('header .user span').text(_partials_js_contacts_js__WEBPACK_IMPORTED_MODULE_1__.user.name); // we add the contacts in the aside

  addContacts(_partials_js_contacts_js__WEBPACK_IMPORTED_MODULE_1__.contacts); // we set the first contact as the current contact

  setAsCurrent($('.contact').eq(0)); // when we click on a contact in the aside
  ...
  function appendMessages(contact) {
    ...
    $('.container-messages').remove(); // we prepend to the div with id "chat" a messages' container having the currentIndex as id
  ...
  function setAsCurrent(contact) {
    ...
    $('.contact').removeClass('current'); // we add the class "current" to the given contact
  ...
  function sendMessage(message) {
    ...
    $('#send-message input').val(''); // we get the current time

    var now = dayjs(); // we add a new message to the currentContact

    addMessage(message, now.format('DD/MM/YYYY H:mm:ss'), 'received', currentContact); // we append to the messages' container a div corresponding to this new message
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunkjquery_boolzap"] = self["webpackChunkjquery_boolzap"] || [];
